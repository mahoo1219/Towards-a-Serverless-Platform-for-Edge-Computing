...
\hyphenation{op-tical net-works semi-conduc-tor ini-tia-li-za-tion he-te-ro-ge-neous}
...
% in the abstract
\begin{abstract}
The emergency of real-time and data-intensive applications, for which high latency and low throughput are prohibitive, challenges the success of the centralized data centre model. The paradigm of fog/edge computing aims to solve these problems; in contrast with cloud data centres, fog nodes are geographically distributed in proximity with end-users, taking advantage of the emerging wireless communication technologies and mobile networks infrastructure.
%and prompting an efficient management of limited resources at each node.
%Fog nodes exhibit limited resources
The resource limitations of densely distributed fog nodes also prompts an efficient usage by applications and services. Addressing this challenge and the needs of different applications scenarios, in this paper we propose a serverless platform for edge computing. 
Firstly, we motivated the adoption of a \textit{serverless architecture} and the \textit{Function-as-a-Service} model.
%for the efficient provisioning of computing services.
...
%of these tools 
%and 
the feasibility of the \textit{Serverless Edge Platform} in satisfying different types of application requirements in heterogeneous deployments.
...
%This work was supported with grant by the \textit{National Council for Scientific and Technological Development} (CNPq) - Brazil.
